...
scrape_imd_data <- function(base_urls, time_periods) {
  ...
  for (i in 1:length(base_urls)) {
    ...
    tryCatch({
      ...
      # Find the script'NormalizedVulnerabilityIndex'
...
# 1. Define the vectors for URLs and their corresponding months
...
write.csv(state_month_avg_nvi_heatwaves, "app/state_month_avg_nvi_heatwaves.csv", row.names = FALSE) 
...
# Save the aggregated data to a CSV file for future use
write.csv(state_month_avg_nvi_lightvul, "app/state_month_avg_nvi_lightvul.csv", row.names = FALSE) 
...
# Save the aggregated data to a CSV file for future use ---
write.csv(state_month_avg_nvi_coldwaves, "app/state_month_avg_nvi_coldwaves.csv", row.names = FALSE) 